...
create or replace NONEDITIONABLE PROCEDURE firstIsFriendSecond(id1 IN jogador.id%TYPE, nick1 IN jogador.nickname%TYPE, id2 IN jogador.id%TYPE, nick2 IN jogador.nickname%TYPE)
    IS
    qtd NUMBER;
    ...
    END;
 --------------------------FUNÇÃO KDA--------------------------
 create or replace NONEDITIONABLE FUNCTION KDA (abates NUMBER,assistencias NUMBER, mortes NUMBER) RETURN
NUMBER IS
    kda NUMBER;
BEGIN
...
create or replace NONEDITIONABLE FUNCTION KDAP(ID participa.id%TYPE,NICK participa.nickname%TYPE,COD partida.codigo%TYPE ) RETURN
NUMBER IS 
    kdaP NUMBER;
    BEGIN
    ...
    END;
    --------------------------------DICIONÁRIO-----------------------------------------------------------
create or replace NONEDITIONABLE PROCEDURE DICIONARIO(STR VARCHAR2) 
IS
BEGIN
    FOR LINHA IN (SELECT * FROM (SELECT P.NOME 
                             FROM PERSONAGEM P
                             UNION
                             SELECT I.NOME
                             FROM ITEM I) UN WHERE UPPER(UN.NOME) LIKE '%'||UPPER(STR)||'%')
    LOOP
