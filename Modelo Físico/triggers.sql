...
CREATE OR REPLACE TRIGGER INSERCAO_PARTICIPA
...
BEGIN
    ...
    FOR reg_pXp IN (SELECT *
                    FROM PARTIDA P1 INNER JOIN PARTICIPA P2 ON P1.CODIGO = P2.CODIGO
                    WHERE P1.CODIGO = :NEW.CODIGO)
    LOOP
        cntr_total := cntr_total + 1;
        IF reg_pXp.TIME = :NEW.TIME THEN
            cntr_time := cntr_time + 1;
        END IF;
        
        IF cntr_time >= 3 THEN
            RAISE_APPLICATION_ERROR(-20205, 'Time já está cheio\n');
        ELSIF cntr_total>=6 THEN
            RAISE_APPLICATION_ERROR(-20205, 'Partida já está lotada\n');
        END IF;
    END LOOP;
    ...
    IF is_in = 0 THEN
        RAISE_APPLICATION_ERROR(-20205, 'Jogador não possui esse personagem\n');
    END IF;
END;
