...
IN (SELECT O.ID,O.NICK
            FROM OBTEVE O
            WHERE O.NOME = (SELECT NOME
...
FROM JOGADOR J
WHERE (J.ID,J.NICKNAME) 
NOT IN (SELECT P.ID,P.NICK
            FROM POSSUI P
            WHERE P.NOME = (SELECT NOME
FROM (SELECT NOME,COUNT(*)
      FROM POSSUI
      GROUP BY NOME
      ORDER BY COUNT(*) DESC
...
SELECT ID,NICKNAME,COUNT(TIME_V) AS VITORIAS
    FROM (SELECT A.ID,A.NICKNAME,A.TIME,A.CODIGO,B.TIME_V
...
FROM     (SELECT     ID, NICKNAME, SUM(KILL) AS S_KILL, SUM(DEATH) AS S_DEATH, SUM(ASSIST) AS S_ASSIST
                FROM    PARTICIPA
                WHERE     KILL IS NOT NULL AND DEATH IS NOT NULL AND ASSIST IS NOT NULL
                GROUP BY (ID, NICKNAME)
        )
...
--- CONSULTA OS ATRIBUTOS DOS PERSONAGENS DEFENSIVOS
SELECT P.NOME, P.HP, P.ATAQUE, P.PODER, P.ARMADURA, P.V_ATAQUE, P.V_MOVIMENTO, D.RESISTENCIA
FROM PERSONAGEM P INNER JOIN DEFENSIVO D
    ON P.NOME = D.NOME
...
HAVING AVG(KDA(P.KILL,P.ASSIST,P.DEATH)) > (Select AVG(KDA(P2.KILL,P2.ASSIST,P2.DEATH)) 
                 FROM PARTICIPA P2)
